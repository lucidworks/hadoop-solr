...
public class CSVIngestMapperTest extends BaseIngestMapperTestCase {
  ...
  public void setUp() throws IOException {
    ...
    //configuration.set(CSVIngestMapper.DATASOURCE, "csv");
  ...
  public void test() throws Exception {
    ...
    LWDocumentWritable val = TestUtils
        .createLWDocumentWritable("id-1", "bar", "The quick brown fox", "junk", "jumped", "zen",
            "head", "hockey", "gretzky", "field_5", "extra");//, BaseHadoopIngest.DATA_SOURCE_FIELD,
    //            "csv", BaseHadoopIngest.BATCH_ID_FIELD, "batchid",
    //            BaseHadoopIngest.DATA_SOURCE_TYPE_FIELD, "ds_type",
    //            BaseHadoopIngest.DATA_SOURCE_PIPELINE_FIELD, "hadoop_solr",
    //            BaseHadoopIngest.DATA_SOURCE_COLLECTION_FIELD, "collection");
  ...
  public void testOptions() throws Exception {
    ...
    LWDocumentWritable val = TestUtils
        .createLWDocumentWritable("id-0", "bar", "bar", "junk", "junk", "zen", "zen", "hockey",
            "hockey");//, BaseHadoopIngest.DATA_SOURCE_FIELD, "csv", BaseHadoopIngest.BATCH_ID_FIELD,
    //            "batchid", BaseHadoopIngest.DATA_SOURCE_TYPE_FIELD, "ds_type",
    //            BaseHadoopIngest.DATA_SOURCE_PIPELINE_FIELD, "hadoop_solr",
    //            BaseHadoopIngest.DATA_SOURCE_COLLECTION_FIELD, "collection");
    ...
    /*
     * Document foo = table.getDocument("id-0", "foo",
    ...
    mapDriver.runTest();
    /*
     * foo = table.getDocument("id-1", "foo", DocumentTable.DocumentFields.TEXT
     * | DocumentTable.DocumentFields.FIELD); assertNotNull("document is null",
     * foo); assertNotNull(foo.fields.get("hockey"));
     * assertNotNull(foo.fields.get("field_5"));
     */
  }
  ...
  public void testVariations() throws Exception {
    ...
    for (PipelineField field : fields) {
      Assert.assertTrue(field.getName().startsWith("field_")
              || field.getName().startsWith(BaseHadoopIngest.LW_PREFIX));
      i++;
    }*/
  }
  ...
  public void testStrategy() throws Exception {
    ...
    //    conf.set(CSVIngestMapper.CSV_DELIMITER, "\t");
    //    conf.set(CSVIngestMapper.CSV_STRATEGY, CSVIngestMapper.TAB_DELIM_STRATEGY);
    //    value = new Text("id-0\tbar\tjunk\tzen\thockey");
    //    mapDriver.withInput(key, value);
    //    run = mapDriver.run();
    //    Assert.assertEquals(1, run.size());
    //    doc = run.get(0).getSecond().getLWDocument();
  ...
  public void testFrankenstein() throws Exception {
    ...
    InputStream frank = CSVIngestMapperTest.class.getClassLoader().getResourceAsStream("frank.csv");
